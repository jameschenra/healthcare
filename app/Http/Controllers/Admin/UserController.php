...
class UserController extends Controller
{
    public function index()
    {
        $users = User::leftJoin('users as uo', 'users.primary_id', 'uo.id')
            ...
            ->leftJoin('membership_pendings as mp', 'users.id', 'mp.user_id')
        ...
        if ($user) {
            ...
            if ($userInfo) {
                ...
                if ($membership) {
                    ...
                    if (($membership && $planType > $membership->plan_type_id)) {
                        session([
                            'checkout_info' => [
                                'type' => 'UPGRADE',
                                'id' => $id,
                                'play_type_id' => $planType
                            ]
                        ]);

                        return redirect()->route('admin.payment.upgradeinfo');
                    }
                } else {
                    $ownerId = null;
